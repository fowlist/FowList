...
if ($bookSelected && $bookTitle <> "")
{
    ...
    $optionsWeapon = $conn->query("
        SELECT  DISTINCT teams, weapon
        FROM    weapons 
        RIGHT   JOIN ( SELECT DISTINCT weapon, teams,image
                FROM    platoonOptions  ) as oW
            ON      weapons.weapon = oW.weapon");
...
<head>
    ...
    <script>import { jsPDF } from "jspdf";</script>
...
<?php include "menu.php"; ?>
    ...
    <script>
        // Your custom JavaScript code using jsPDF
...
<!--<a href="index<?=$beta?>.php?<?php echo $backQuery ?>">Back</a> -->
    <div class="header">
        <h2 class="<?=$query['ntn']?>"> 
            <?=$bookTitle?>    
            <div class='Points'>
...
if ($bookSelected) {
    ...
    // --- set general box nr variable
            ...
            if (($row["formation"] == $currentFormationCode) && ($query[$currentBoxInFormation] === $row["platoon"])) {
                $boxNrInFormation++;
    ...
    //------- check if the formation have cards for the box --------------
                        ...
                        $boxAllHTML[$currentBoxInFormation] .= $flag . $temp1 . $platoonCardMod[$platoonIndex]["image"];
                    }
                }
    ...
    //---------------- name etc. -------------  
                ...
                $platoonsInForce[$platoonIndex]["title"] = ((($formationCardTitle[$formationNr] != "")&&(!is_numeric(strpos($platoonsInForce[$platoonIndex]["title"],$formationCardTitle[$formationNr]))))? $formationCardTitle[$formationNr] . ": ": "") . $platoonsInForce[$platoonIndex]["title"];
                $boxAllHTML[$currentBoxInFormation] .= "
                    <div  class='title'>
                        <b> 
    ...
    // ------ Config of platoon -------------      
                $boxAllHTML[$currentBoxInFormation] .= $configHTML . $optionsHTML . $formationCardHTML . $platoonCardChange[$platoonIndex]["html"] . $cardsHTML;
                    // -------- Points ----------  
                $boxAllHTML[$currentBoxInFormation] .= "
...
//Card platoon 
    ...
    //----- generate boxes
        foreach ($combinedSupportDB as $row) {
            ...
            if (($query[$currentBoxInFormation] === $row["platoon"])) {
    ...
    //------- check if the formation have cards for the box --------------
                        $formationCardHTML = printFormationCardsHTML($formationCards, $formationCardTitle, $formationNr, $platoonIndex, $currentBoxNr, $platoonCardMod, $row, $row3, $query, $CardsInList, $platoonsInForce, $attachmentsInForce);
                        // ------------ platoon cards (pioneer etc.)
                        $flag =  generateTitleImanges($insignia, $platoonCardMod[$platoonIndex]["title"] . $platoonsInForce[$platoonIndex]["title"] , $query['ntn']);
                        $boxAllHTML[$currentBoxInFormation] .= $flag . $temp1 . $platoonCardMod[$platoonIndex]["image"];
                    }
                }
                mysqli_data_seek($platoonConfig ,0);  
    ...
    // ------ Config of platoon -------------      
                    $boxAllHTML[$currentBoxInFormation] .= $currentConfig . $optionsHTML . $formationCardHTML . $platoonCardChange[$platoonIndex]["html"] . $cardsHTML;
                    $boxAllHTML[$currentBoxInFormation] .= "
                <div class='Points' onclick='togglePoints(this)'>
                    <div>
                        " . $boxCost[$formationNr][$currentBoxNr] . " points
...
//------- check if the formation have cards for the box --------------
                    $formationCardHTML = printFormationCardsHTML($formationCards, $formationCardTitle, $formationNr, $platoonIndex, $currentBoxNr, $platoonCardMod, $row, $row3, $query, $CardsInList, $platoonsInForce, $attachmentsInForce);
                    // ------------ platoon cards (pioneer etc.)
                    $flag =  generateTitleImanges($insignia, $platoonCardMod[$platoonIndex]["title"] . $platoonsInForce[$platoonIndex]["title"], $row["Nation"]);
                    $boxAllHTML[$currentBoxInFormation] .= $flag . $temp1 . $platoonCardMod[$platoonIndex]["image"];
                }
            }
            mysqli_data_seek($platoonConfig ,0);  
...
foreach ($platoonsInForce as $key => $row1) {
    foreach ($row1 as $subrow) echo "<!--{$subrow}-->";
    if (($row1['originalPlatoonCode'] != "")||($row1["replaceImgWith"] !="")) {
    ...
    foreach ($platoonSoftStats as $row2) { 
        if (( $row1['code'] === $row2["code"])&&($row2["TACTICAL"]!="")) {
            ...
            <b><span class='left'>" . generateTitleImanges($insignia, $cardTitle . (($row1["title"]=="")? $row2["title"] : $row1["title"] ), ($row1["Nation"]<>"")?$row1["Nation"]:$query['ntn']) ."</span><span>".$cardTitle .(($row1["title"]=="")? $row2["title"] : $row1["title"] )." </span></b><br>
                " . $row2["Keywords"]."</td>
            <td rowspan='2'>";
//------------------ Motivation ------------------------
...
//-------------------Save ------------------------------------
            ...
            echo "<td rowspan='2'>";
                ...
                if ($platoonCardMod[$row1['platoonIndex']]['isHitOn']!=""&&(!isset($row1["attachment"]))) {
                    $platoonIsHitOn = "<div class='floatingImg'><img src='img/cardSmall.svg'></div>" . $platoonCardMod[$row1['platoonIndex']]['isHitOn'];
                } elseif ($platoonCardMod[$row1['platoonIndex']]["attachment"]['isHitOn']!=""&&(isset($row1["attachment"]))) {
                ...
                if (($platoonCardMod[$row1['platoonIndex']]["replaceSave"] == "save")){
...
//--------------------Speed --------------------------------------
            if (($platoonCardMod[$row1['platoonIndex']]["replaceMovement"] == "movement")){
...
foreach ($weaponsTeamsInForce as $row1) if (( $row1 <> "")&&( $row1 <> "Komissar team")&&( $row1 <> "Komissar Team")&&( $row1 <> "komissar team")) {               
    ...
    if (isset($weaponsRow)) {
        ...
        if ( ($row1 === $row2["team"])||($row1 === $row2["teams"]) ) {
                    
                    foreach ($rules as $row3) {
                        if (is_numeric(strrpos(strtoupper($row2["notes"]), strtoupper($row3["name"]),-1))) {
                            $rulesInForce[] =  $row3["name"];
                        }
                    }                
